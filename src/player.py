...
class Player(Entity):
    ...
    def update(self):
        ...
        if self.vector[0] != 0:
            self.current_speed[0] = min(self.current_speed[0] + self.acceleration, self.max_speed)
        else:
            self.current_speed[0] = max(self.current_speed[0] - self.acceleration, 0)
        if self.vector[1] != 0:
            self.current_speed[1] = min(self.current_speed[1] + self.acceleration, self.max_speed)
        else:
